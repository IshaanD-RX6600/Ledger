...
const inputCls =
  "rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 placeholder:text-gray-400 w-full sm:w-auto";
...
export default function AddHolding({ onAdd }: { onAdd: (h: Holding) => void }) {
  ...
  return (
    <div className="rounded-xl bg-white p-4 shadow-sm border border-gray-100">
      {/* Mobile: 2-column grid (symbol full-width, shares+cost side-by-side, Add full-width) */}
      ...
      <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-end sm:gap-3">
        <div ref={containerRef} className="relative col-span-2">
          ...
          {open && results.length > 0 && (
            <ul className="absolute z-50 mt-1 w-full sm:w-56 rounded-xl border border-gray-100 bg-white shadow-lg overflow-hidden">
              {results.map((r) => (
                <li
                  key={r.symbol}
        ...
        <button
          onClick={submit}
          className="col-span-2 w-full sm:w-auto rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700"
        >
          Add
        </button>
      </div>
